...
final class RestaurantManager {
    ...
    func getAllRestaurants() async throws -> [Restaurant] {
        [
            ...
            Restaurant(id: "4", title: "Local Farmer", cuisine: .american),
        ]
    }
}
...
struct SearchableBootcamp: View {
    ...
    var body: some View {
        ScrollView {
            ...
            // instead of computed property in vm, you can also use environment value from SwiftUI, but the limitation is that the value needs to be read in child view
//            Text("ViewModel is searching: \(viewModel.isSearching.description)")
//            SearchChildView()
        }
        
        // a modifier with built-in searchBar interface
        .searchable(text: $viewModel.searchText, placement: .automatic, prompt: Text("Search restaurants..."))
        
        // a modifier with built-in filter search result interface
        ...
        // a modifier with built-in search suggestion association
        .searchSuggestions({
